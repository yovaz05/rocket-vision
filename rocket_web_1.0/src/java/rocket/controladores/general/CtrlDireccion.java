...
public class CtrlDireccion extends GenericForwardComposer {
   ...
   */
  //para que haga la consulta en la bd 1 sóla vez
   ...
   */
  private void getCiudadesPorEstado() {
    cargarCiudades();
   ...
   */
  private void getZonasPorCiudad() {
    cargarZonas();
  ...
  boolean actualizarZona() {
    ...
    if (Sesion.esVistaCelula()) {
      ...
      getIdLider();
      ok = servPersona.actualizarIdZona(idLider, idZona);
    }
    return ok;
  }
...
//TODO: validar si zona nueva no existe en la base de datos
  ...
  public void onClick$etqCiudad() {
    ...
    getCiudadesPorEstado();
    //se trae la ciudad actual para seleccionar en la lista
    ciudad = etqCiudad.getValue();
    activarEditCiudad();
  }
  ...
  boolean actualizarDetalleDireccion() {
    ...
    if (Sesion.esVistaCelula()) {
      ...
      //actualizar detalle de dirección de líder
      getIdLider();
      ok = servPersona.actualizarDireccionDetalle(idLider, detalle);
    }
    return ok;
  }
